...
func (a *Agent) connectivityChecks() {
	...
	contact := func() {
		if err := a.run(a.context(), func(ctx context.Context, a *Agent) {
			...
			case ConnectionStateChecking:
				a.log.Tracef("contact ConnectionStateChecking, %d, %d", a.disconnectedTimeout.Milliseconds(), a.failedTimeout.Milliseconds())
	...
	a.log.Tracef("Starting contact monitoring")
	for {
		...
		a.log.Tracef("Contact interval: %d", interval.Milliseconds())
		t := time.NewTimer(interval)
		select {
		case <-a.forceCandidateContact:
			a.log.Tracef("Force candidate contact")
			t.Stop()
			contact()
		case <-t.C:
			a.log.Tracef("Contact at interval %d", interval)
			contact()
		case <-a.done:
			a.log.Tracef("Contact done, stopping")
			t.Stop()
			return
		}
	}
